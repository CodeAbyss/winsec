...
int wmain(int argc, wchar_t *argv[]) {
  ...
  if (argc < 5) {
    ...
    wprintf(L"\n    -b <machine>          : Remote machine with database (copied to local machine)");
    wprintf(L"\n    -u <domain\\username>  : Username for remote machine");
    wprintf(L"\n    -p <password>         : Password for remote machine\n");
    wprintf(L"\n    -l                    : Dump local SAM accounts of remote machine while you're at it, buddy!");
    wprintf(L"\n    -c                    : Dump Domain Cached Credentials of remote machine.\n");
