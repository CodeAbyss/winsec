...
/**
 *
 *  mainly for resolving hostnames,ip addresses and testing connectivity
 *  Works similar to ping program
 *
 *  Copyright (c) 2010 -   <???@???>
 *
 */
...
Network::Network(class Machine* host)
{
  this->host = host;
...
{ 
  ...
  if(GetAddrInfo(host->GetName(),NULL,&aiHints,&aiList) == 0) {
  
    // if we get anything, we only return the first entry in list
    if(aiList != NULL) {
